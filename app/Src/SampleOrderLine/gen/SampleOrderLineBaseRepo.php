...
use App\Src\SampleOrderLine\gen\SampleOrderLineFieldValidator;
...
class SampleOrderLineBaseRepo extends BaseRepo
{
  ...
  public function createRec(array $rec)
  {
    ...
    $validator = Validator::make(
      $rec,
      SampleOrderLineFieldValidator::getCreateRules()
    );
    if ($validator->fails()) {
    ...
    $object =  SampleOrderLine::create($rec);
    if ($this->modelAuditable('SampleOrderLine')) {
  ...
  {
    $object = SampleOrderLine::findOrFail($model_id);
    ...
    $validator = Validator::make(
      $rec,
      SampleOrderLineFieldValidator::getUpdateRules($model_id)
    );
    if ($validator->fails()) {
      throw new Exception($validator->errors());
    }
    ...
    $_is_auditable = $this->modelAuditable('SampleOrderLine');
    if ($_is_auditable) {
  ...
  public function query($request)
  {
    ...
    $data = SampleOrderLine::orderBy('_seq')
      ->get()
      ->map(
    ...
    return SampleOrderLineView::collection($data);
  }
  ...
  public function showRec($model_id)
  {
    ...
    return SampleOrderLineWithParentsView::collection($data);
  }
}
